...
namespace NESTCOOKING_API.DataAccess.Repositories
{
	public class StatisticRepository : IStatisticRepository
	{
		...
		public async Task UpdateAllStatistics()
		{
			...
			var totalRevenue = (await _context.Users.ToListAsync()).Where(u => u.Email == AppString.MailEmail).FirstOrDefault()?.Balance;
			var currentRevenueStatistic = await _context.RevenueStatistics.FirstAsync(rs => rs.Date == currentDateOnly);
			currentRevenueStatistic.Revenue = (double)(totalRevenue == null ? 0 : totalRevenue);
			_context.RevenueStatistics.Update(currentRevenueStatistic);

			var totalBooking = (await _context.Bookings.ToListAsync()).Where(b => b.Status == StaticDetails.ActionStatus_ACCEPTED || b.Status == StaticDetails.ActionStatus_COMPLETED).Count();
			var currentBookingStatistic = await _context.BookingStatistics.FirstAsync(bs => bs.Date == currentDateOnly);
			currentBookingStatistic.TotalOfBooking = totalBooking;
			_context.BookingStatistics.Update(currentBookingStatistic);
